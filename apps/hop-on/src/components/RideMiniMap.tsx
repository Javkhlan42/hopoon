...
export function RideMiniMap({
  ...
  useEffect(() => {
    const apiKey = process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY || 'AIzaSyDTwVUsWOt5IFdO6ZYj-oRoYkyV3cQ2wmY';
    ...
    if (originLat && originLng) {
      ...
      markers.push(`color:0x00C853|label:A|${encodeURIComponent(originAddress)}`);
    }
    
    // Destination marker (red with B)
    if (destinationLat && destinationLng) {
      markers.push(`color:0xE53935|label:B|${destinationLat},${destinationLng}`);
    } else if (destinationAddress) {
      markers.push(`color:0xE53935|label:B|${encodeURIComponent(destinationAddress)}`);
    }
    ...
    setMapUrl(url);
  }, [originAddress, destinationAddress, originLat, originLng, destinationLat, destinationLng]);
    ...
    <div className={className}>
      ...
      <div className="bg-white rounded-b-lg border border-t-0 border-gray-200 p-4 shadow-sm">
        <div className="flex items-center gap-3 mb-3">
          ...
          <span className="text-sm text-gray-700 truncate flex-1">{originAddress}</span>
        </div>
        <div className="flex items-center gap-3">
          <div className="w-3 h-3 rounded-full bg-red-500 flex-shrink-0"></div>
          <span className="text-sm text-gray-700 truncate flex-1">{destinationAddress}</span>
        </div>
      </div>
    </div>
  );
